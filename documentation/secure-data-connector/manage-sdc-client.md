...
If there is only one SDC client instance is running, it is recommended that you create a new SDC client. This ensures no disruptions to existing connections. Refer to [Run the Secure Data Connector Client under Proxy Network Setup](configure-sdc-client#deploy-the-secure-data-connector-client) for more information. 
